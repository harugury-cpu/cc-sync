...
## Running a Task
1. Use `gpt-5.5` as the default model unless the user explicitly requests a different model. Ask the user (via `AskUserQuestion`) only for the reasoning effort (`xhigh`, `high`, `medium`, or `low`). If the user specifies a different model (e.g. `gpt-5.4`), use that instead. (`gpt-5.3-codex` has been migrated to `gpt-5.4`.)
