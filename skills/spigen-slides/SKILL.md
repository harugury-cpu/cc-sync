...
> V6.0 변경: Spigen Design System 컬러 토큰 직접 동기화
> - DARK surface #0E0E0E → #262626 (24% 밝게)
> - LIGHT accent #FF6B1A → #EF5F0E (어두운 오렌지, 흰 배경 가독성)
> - LIGHT bg 순백 → #F4F2EC (워밍 오프화이트)
> - good/bad/accent_strong/accent_line/surface_hi/surface_2 토큰 신규
> 출처: `~/Downloads/Spigen Dark|Light Design System/colors_and_type.css`
...
- **기본 테마: dark (V5.7)** — `theme="dark"` 가 default. light는 사용자가 "라이트로", "light theme" 같이 명시 요청 시에만 사용.
- **표지 제목 2줄 이내 (V5.7)**: `cover(title=...)` 의 `\n` 개수 1개 이하. 3줄 이상 시 자동 트림 또는 경고.
...
**선택 원칙 (V5.4 — 시트는 결과 레퍼런스, 입력 슬롯이 아니다):**
...
| 타입 | 판단 기준 | 사용 방식 | 방식 |
|-----|---------|------------|-----|
| 단순 서술/나열 | 구조화 어려운 설명·주의사항·요약 본문 | `slide()` | V5 |
| 단일 강조 메시지 | 한 슬라이드 = 한 문장 강조 | `callout()` | V5.4 |
| 완료·미완료 체크리스트 | 항목별 완료 여부 목록 | `checklist()` | V5 |
| 자유 카드 조합 | 항목 수가 4개+ 또는 시트 슬롯과 다른 카드 그룹 | `start_slide()` + `card()` 반복 | V5.4 |
| 자유 플로우 조합 | 단계 7개+ 또는 자유 형태 단계 | `start_slide()` + `flow_step()` 반복 | V5.4 |
| 자유 비교 조합 | 비교 6행+ 또는 비교 + 결론을 한 슬라이드에 | `start_slide()` + `compare_pair()` 반복 | V5.4 |
| **시트 슬롯 정확 일치 — Before/After** | 항목별 before→after, 정확히 1~5행 | `mk_compare_rows` | V4 |
| **시트 슬롯 정확 일치 — 프로세스 강조** | 정확히 3~6단계, 동일 형태 | `mk_flow_focus` | V4 |
| **시트 슬롯 정확 일치 — 프로세스 밀도** | 6단계 이상, 비용/시간 포함 | `mk_flow` | V4 |
| **시트 슬롯 정확 일치 — 팀 매핑** | 팀(행) × 항목(열), **정확히 3행 이하** | `mk_swimlane_mapping` | V4 |
| **시트 슬롯 정확 일치 — 3카드** | 정확히 3개 동등 항목 | `mk_3col_cards` | V4 |
| **시트 슬롯 정확 일치 — 분기** | YES/NO 두 갈래 결정 | `mk_decision_tree` | V4 |
| **시트 슬롯 정확 일치 — 레이어** | 입력→처리→출력 레이어 | `mk_arch_layers` | V4 |
| KPI 진행 현황 | KPI / 목표 / 실적 / 달성률 | `kpi_status()` ★ template="kpi" | V5 |
...
> 빌드 결과의 품질 심층 분석. **모든 빌드 후 자동 호출** (default ON).
> 사용자가 "검수 생략" / "skip review" 같이 명시 거부하면 OFF.

### 동작 (default ON)

빌드 완료 → 자동으로 3종 페르소나 검수 진행 → 의견 종합 + 충돌 시 사용자 1회 문의 → 1회 통합 수정 → 종료.

### 검수 생략 트리거 (사용자 명시 시 OFF)

다음 표현이 사용자 메시지에 있으면 검수 생략:
...
| `spigen_subagent_prompts.md` | 페르소나 검수 프롬프트 (V5.6+) + 영역 매트릭스 |
...
> V6.0.1: 디자인 룰은 빌더 코드(`spigen_build.py` / `spigen_lib.py` / `spigen_tokens.py`)가 자동 강제하고, 코드로 못 잡는 텍스트·메시지·청중 차원 룰은 `SKILL.md` 디자인 금지 9카테고리 + `spigen_subagent_prompts.md` 페르소나 영역 매트릭스에 명시. 별도 룰 .md 파일 불필요 (V6.0.1에서 정리).
