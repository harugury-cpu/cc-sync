...
[2] (슬라이드 제목)
    레이아웃: (컴포넌트 유형 — 예: 카드 3개 / 번호 단계 5개 / 비교 2단)
    텍스트: (어떤 내용이 어디에 — 예: 상단 헤더 "제목", 카드별 항목 설명)
    이미지: 없음

[3] (슬라이드 제목) [이미지 필요]
    레이아웃: (예: 좌 텍스트 40% + 우 이미지 60%)
    텍스트: (좌측 — 어떤 내용)
    이미지: 📎 우측 영역 (권장 비율 16:9, 최소 800px 이상)
            → 이미지를 여기에 붙여넣으면 해당 위치에 삽입됩니다.

승인하시면 생성을 시작합니다.
이미지가 필요한 슬라이드는 이미지를 먼저 제공해주시면 함께 진행합니다.
```

**레이아웃 설명 규칙:**
- 모든 슬라이드에 "텍스트: ..." 줄을 포함해 어떤 내용이 어디에 배치되는지 명시한다.
- 이미지가 필요한 슬라이드는 `[이미지 필요]` 태그 + 📎 이미지 영역 설명을 붙인다.
- 이미지 영역 설명에는 위치(좌/우/전체/상단 등), 권장 비율, 어떤 이미지가 와야 하는지를 적는다.
- 이미지 없이 진행할 경우 대체 텍스트 컴포넌트로 처리함을 명시한다.
