...
```
Step 1. 내용 수집
Step 2. 슬라이드별 브리프 작성 → 사용자 승인  ← 품질의 원천 (시각·내용 둘 다 여기서 결정)
Step 3. Preflight Gate 통과
Step 4. 생성 + 링크 공유
Step 5. 검증 실행 여부 질문
```

> **이 스킬의 단 하나의 핵심**: 슬라이드는 컴포넌트에 내용을 채워 만드는 것이 아니라, **브리프(주장·시각 설계)를 먼저 쓰고 그 브리프를 빌드하는 것**이다. "내용 → 컴포넌트 매핑"은 답습을 낳는다. 반드시 "내용 → 브리프 → 시각 설계" 순서. 컴포넌트 목록은 브리프의 시각 아이디어를 *구현할 도구를 찾을 때만* 참고하는 인벤토리이지, 내용을 끼워 넣을 슬롯이 아니다.
...
> ⚠️ **아래 표는 "결정 테이블"이 아니라 "참고 인벤토리"다.** 내용 타입을 보고 컴포넌트를 lookup하지 않는다 — 그것이 답습의 원인이다. **반드시 Step 2 브리프(주장·위계·시선 흐름·영역 분할)를 먼저 쓰고**, 그 시각 아이디어를 실제 빌더 메서드로 옮길 때만 아래에서 도구를 고른다. 같은 내용 타입이라도 브리프의 주장·위계가 다르면 다른 시각이 나와야 정상이다.

중간 슬라이드 컴포넌트 인벤토리 (브리프의 시각 아이디어를 구현할 도구 참고용).
...
## Step 2. 슬라이드별 브리프 작성 (품질의 원천 — 시각·내용 둘 다 여기서 결정)

제목 한 줄짜리 "구성 제안"으로 끝내지 않는다. **슬라이드마다 아래 브리프를 빌드 전에 써서 사용자 승인을 받는다.** 이 브리프가 없으면 빌드하지 않는다. 시각 답습(컴포넌트 재사용)도, 내용 빈약(주장 없는 정보 나열)도 전부 이 단계를 건너뛰어서 생긴다.

### 슬라이드 브리프 양식 (장마다 작성)

```
[N] (슬라이드 제목) — eyebrow: CATEGORY
주장: 이 슬라이드가 증명하는 단 한 문장. "그래서 뭐?"에 답이 되어야 한다.
근거: 주장을 뒷받침하는 원본 사실·수치 (원본에 없는 것 창작 금지)
위계: 1순위 → 2순위 → 3순위 (무엇이 가장 크고/먼저 보여야 하나)
시각 아이디어: 주장·위계를 어떻게 화면으로 표현할지 — 컴포넌트 이름이 아니라 "어떻게 보이는가"로
시선 흐름: 눈이 어디서 시작해 어디로 가는가 (①→②→③)
영역 분할: 화면을 무슨 기준으로 쪼개나 — 분할 시각 장치는 하나만 (세로 그룹핑 + 가로 분할 동시 금지)
합격 기준: 빌드 후 O/X로 채점할 2~3개 (사전 고정 — 빌드 후 이것만 본다)
```

### ★ 주장 게이트 (내용 충실도의 핵심)

- **모든 콘텐츠 슬라이드는 "주장" 한 문장이 있어야 한다.** 주장이 안 써지는 슬라이드는 단계 나열·정보 덤프이지 슬라이드가 아니다. 주장을 못 쓰면 그 슬라이드를 합치거나 버린다.
- 주장은 "X는 Y다 / X로 Z가 된다"처럼 **결론**이어야 한다. "X의 단계 / X의 구성"은 주장이 아니라 목차다.
- **내용 충분성 게이트**: 원본을 읽어도 주장을 뽑을 근거가 부족하면 **빌드를 멈추고 사용자에게 되묻는다** — "이 내용으로 무엇을 말하고 싶으세요? 결론이 무엇인가요?". AI가 원하는 바를 모른 채 컴포넌트를 채우지 않는다 (AI는 내용을 창작하지 않는다 원칙의 입구 적용).

### 시각 설계 규칙 (시각 답습 차단)

- 시각 아이디어는 **컴포넌트 이름으로 적지 않는다.** "flow_step 4개"가 아니라 "왼쪽 원인 → 오른쪽 결과로 읽히는 가로 흐름, 결과만 강조"처럼 *어떻게 보이는가*로 적는다. 컴포넌트는 그 다음 인벤토리에서 고른다.
- 같은 내용 타입이라도 주장이 다르면 다른 시각이 나와야 한다. **직전 덱에서 본 레이아웃을 그대로 가져오면 답습** — 이 내용 고유의 주장에서 화면을 새로 도출한다.
- 카드 사용은 핵심 원칙의 "카드 사용 문법"을 따른다 (오픈 기본, 강조·묶음만 카드, 나열·빈 박스 금지).

승인 형식:
```
=== 슬라이드별 브리프 ===
[1] 표지 — (제목)  ※ 표지는 템플릿 복사, 브리프 대상 아님
[2] (제목) — 주장: ... / 시각: ... / 합격: ...
[3] ...
승인하시면 생성을 시작합니다.
```
