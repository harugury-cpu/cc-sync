...
- **테마 선택**: `theme="light"` (기본) 또는 `theme="dark"` — 각각 별도 지정 템플릿 cover 사용.
...
> 빌드 결과의 품질 심층 분석이 필요할 때 호출하는 보조 검수 시스템.
> default는 OFF — 사용자가 명시 호출할 때만 동작.

### 호출 트리거 (사용자 명시 시에만)

다음 키워드/표현이 포함되면 페르소나 검수 모드 진입:
- "검수 부탁", "페르소나 검토", "리뷰 봐줘", "review"
- "기획자 / 디자이너 / 청중 관점에서 봐줘"

명시 호출이 없으면 페르소나 호출하지 않는다 (자동 호출 금지).
