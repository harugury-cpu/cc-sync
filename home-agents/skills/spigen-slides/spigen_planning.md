...
2. **방식**: "제안서·시안 형태인가요, 보고서·분석 형태인가요?" → 제안서·시안 → 템플릿 방식 / 그 외 → lib 방식
3. **청중**: "누가 보는 자료인가요? (내부 팀 / 경영진 / 외부 고객 등)"
4. **톤**: "어떤 분위기로 전달하고 싶으신가요? (예: 신뢰감 있는 / 데이터 중심 / 설득적인)"
5. **날짜** (언급 없으면 오늘 날짜 자동 사용, 묻지 않음)
6. **버전** (언급 없으면 V1.0, 묻지 않음)
