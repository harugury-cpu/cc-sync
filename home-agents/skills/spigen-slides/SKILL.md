...
생성 결과에서 사용자가 수기로 위치를 정리하면 안 된다.

반드시 지켜야 할 hard rule:

```txt
모든 shape / text box / card / chart / diagram node 는 720 × 405pt 캔버스 안에 있어야 한다.
```
...
- 여유가 충분하면 상단 정렬 유지 가능
- 여백이 빡빡하면 위/아래 여백이 비슷해 보이도록 균형 배치
- 제목/설명 2줄 구조도 하나의 text group 으로 계산
...
- 불렛이 텍스트보다 위로 뜨면 안 됨
- 불렛이 첫 줄보다 아래로 처지면 안 됨
- 불렛 간격과 텍스트 시작점은 같은 컴포넌트 안에서 고정값 사용

추가 원칙:

```txt
카드 계열에서는 불렛을 쓰지 않는다.
작은 카드 / 3열 카드 / compact info card 에는 불렛 금지.
불렛은 넓은 본문 블록이나 checklist 영역에서만 예외적으로 허용한다.
```
...
`mk_flow()`는 **압축형 생성 플로우, 실행 순서, 처리 단계**를 보여주는 기본 컴포넌트다.
...
동작흐름 자체가 페이지의 핵심이고, 아래에 비용표/요약/복합 표가 붙지 않는다면  
작은 `mk_flow()`보다 **한 줄 카드형 템플릿 흐름 레이아웃**을 우선 사용한다.
...
```txt
단순 흐름만 필요하되 flow가 메인 메시지 → mk_flow_focus()
단순 흐름만 필요하고 보조 설명 수준 → mk_flow()
흐름 + 비용표 + 운영 금액 + summary 필요 → 템플릿 5페이지 참고/복제
```

구조 설명은 항상 표/카드 나열보다 **다이어그램 계열 컴포넌트**를 우선한다.

```txt
구조 / 계층 / 관계 → mk_arch_layers(), mk_arch_orchestrator(), mk_decision_tree(), mk_swimlane_mapping()
```

선택 기준:

```txt
추상적인 단계/계층 구조 → mk_arch_layers()
모듈 호출/의존 관계 → mk_arch_orchestrator()
판단/분기/폴백 → mk_decision_tree()
모듈 ↔ 설정 / 데이터 매핑 → mk_swimlane_mapping()
```
