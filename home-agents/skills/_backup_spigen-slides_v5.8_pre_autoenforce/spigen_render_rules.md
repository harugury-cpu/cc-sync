...
권장 콘텐츠 영역: y 128 ~ 381pt (헤더·하단 안전영역 분리)
```
...
```txt
제목 + 설명은 하나의 묶음(group)으로 취급한다.
각 줄을 따로 보지 않고 text group 전체 높이로 계산한다.
```
...
```txt
margin / gap / row spacing은 반복값(토큰)을 사용한다.
눈대중 위치 조정 금지 — 수치로 결정한다.
```
...
한 슬라이드에서 카드 유형은 **1~2종 이하**로 제한한다.

금지:

```txt
비대칭 혼합 박스 (크기·비율이 다른 카드 혼재)
3-up + 1-wide 레이아웃 (3개 세로 카드 + 1개 가로 카드 혼합)
동일 역할인데 다른 비율로 반복
```
...
```txt
핵심 성과는 하나만 전면 강조한다.
여러 KPI 수치를 동등하게 나열하지 않는다.
추이 차트를 쓸 때는 최종값(또는 목표값) 1개를 큰 숫자로 분리한다.
장식형 차트 (그라디언트 fill, 3D, 과도한 애니메이션) 금지.
```
...
```txt
⚠️ 현재 verifier가 더 잘 본다는 이유만으로
더 맞지 않는 컴포넌트로 후퇴하지 않는다.
청중의 인지 과제에 맞는 컴포넌트를 먼저 고르고,
검증이 비어 있으면 verifier를 보강한다.
```
...
| 의도 | 권장 | 지양 |
|------|------|------|
| 원칙 요약 | rule grid | 3-up + 1-wide 혼합 |
| 프로세스 설명 | flow | 2줄 카드 혼합 |
| 구조 설명 | structure (diagram) | 카드 나열 fallback |
| 관계 정리 | mapping (swimlane) | 카드 밖 bar / gutter 요소 |
| 수치 보고 | KPI / chart | 장식형 차트 |

원칙: 의도가 먼저고, 레이아웃은 그 의도에 맞는 컴포넌트를 선택하는 방식으로 결정한다.
