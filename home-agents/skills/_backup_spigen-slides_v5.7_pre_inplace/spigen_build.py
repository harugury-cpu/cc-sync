...
class SpigenBuilder:
    ...
    def cover(self, title, subtitle="", dept="디자인부문ㅣ패키지디자인팀",
        ...
        V5.7: title 2줄 이내 강제 — 3줄 이상이면 자동 트림 + 경고.
        """
        # V5.7 가드: title \n 개수 1개 이하
        title_lines = title.count("\n") + 1
        if title_lines > 2:
            parts = title.split("\n")
            print(f"[WARN] cover title {title_lines}줄 → 2줄로 자동 트림")
            title = "\n".join(parts[:2])
    ...
    def checklist(self, heading, items, eyebrow=""):
        ...
        V5.7: eyebrow 인자 + 헤더 좌표 start_slide와 통일 + 콘텐츠 시작 y=100.
    ...
    def start_slide(self, heading="", eyebrow=""):
        ...
        eyebrow: 타이틀 위 오렌지 메타 텍스트 (8pt bold)
        heading: 22pt bold 슬라이드 타이틀

        텍스트 시각 간격 ≈ 5pt:
          eyebrow box (valign=BOTTOM): y=18~32  → 텍스트 박스 하단에 붙음
          title   box (valign=TOP)   : y=33~63  → 텍스트 박스 상단에 붙음

        eyebrow 없이 heading만 있을 때는 y=20 위치 (기존 동작 유지).
        권장 콘텐츠 시작점: eyebrow 있을 때 y >= 100.
        ...
        if eyebrow:
            eb = _uid()
            # 토큰 기준 (spigen_tokens.HEADER.eyebrow): valign=MIDDLE, 박스 가운데가 텍스트 위치
            self._shape(oid, eb, 40, 32, 320, 10)
            self._text(eb, eyebrow)
            self._style(eb, 8, bold=True, color=self.c["accent"])
            if heading:
                h = _uid()
                # 토큰 기준 (spigen_tokens.HEADER.title_with_eyebrow): valign=MIDDLE
                self._shape(oid, h, 40, 46, 640, 26)
                self._text(h, heading)
                self._style(h, 22, bold=True)
        elif heading:
    ...
    def conclusion(self, metric, caption="", details=None, heading="", eyebrow=""):
        ...
        for i, d in enumerate((details or [])[:4]):
            ...
            if d.get("label"):
                lo = _uid()
                self._shape(sid, lo, 382, cy + 6, 80, 14)
